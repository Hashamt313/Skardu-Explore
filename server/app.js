      ...
      if (fs.existsSync(blogsJsonPath)) {
        ...
        for (const blog of blogs) {
          await pool.query(
            ...
            [blog.id, blog.title, blog.slug, blog.excerpt, blog.content, blog.img, blog.author, blog.category, blog.date, blog.status]
          );
        }
...
const handleRoute = (fn) => async (req, res, next) => {
  try {
    if (!pool) {
      throw new Error('Database connection pool is not initialized. Please configure DATABASE_URL environment variable.');
    }
